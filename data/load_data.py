...
def load_data(config: dict, val_percent=0.8):
    ...
    def get_data_paths(
        ...
        for domain in path_domains:
            path_img = Path(paths_data["path_aerial_" + status], domain)
            for area in os.listdir(os.path.join(DATA_DIR, path_img)):
                aerial = sorted(
                    list(
                        list_items(
                            os.path.join(
                                DATA_DIR,
                                Path(paths_data["path_aerial_" + status])
                                / domain
                                / Path(area),
                            ),
                            "IMG*.tif",
                ...
                sen2sp = sorted(
                    list(
                        list_items(
                            os.path.join(
                                DATA_DIR,
                                Path(paths_data["path_sen_" + status])
                                / domain
                                / Path(area),
                            ),
                            "*data.npy",
                ...
                data["PATH_LABELS"] += sorted(
                    list(
                        list_items(
                            os.path.join(
                                DATA_DIR,
                                Path(paths_data["path_labels_" + status])
                                / domain
                                / Path(area),
                            ),
                            "MSK*.tif",
    ...
    with open(os.path.join(DATA_DIR, paths_data["path_sp_centroids"]), "r") as file:
        matching_dict = json.load(file)

    path_trainval = Path(paths_data["path_aerial_train"])

    trainval_domains = os.listdir(os.path.join(DATA_DIR, path_trainval))
    shuffle(trainval_domains)
    idx_split = int(len(trainval_domains) * val_percent)
    train_domains, val_domains = (
        trainval_domains[:idx_split],
        trainval_domains[idx_split:],
    )

    dict_train = get_data_paths(
        config, train_domains, paths_data, matching_dict, test_set=False
    )
    dict_val = get_data_paths(
        config, val_domains, paths_data, matching_dict, test_set=False
    )
    path_test = Path(paths_data["path_aerial_test"])
    test_domains = os.listdir(os.path.join(DATA_DIR, path_test))
    dict_test = get_data_paths(
        config, test_domains, paths_data, matching_dict, test_set=True
    )

    return dict_train, dict_val, dict_test


def adding_encoded_metadata(
    path_metadata_file: str, dict_paths: dict, loc_enc_size: int = 32
):
    """
