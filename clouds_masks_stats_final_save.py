...
def plot_cloud_coverage_statistics(before, after, series_flags, output_dir):
    ...
    plt.hist(before, bins=15, alpha=0.6, label="Before Filtering", color="gray", edgecolor="black")
    plt.hist(after, bins=15, alpha=0.6, label="After Filtering", color="skyblue", edgecolor="black")
...
def display_most_cloudy_examples(cloudy_examples, output_dir):
    ...
    for idx, (cloud_percent, img) in enumerate(cloudy_examples):
        ...
        img_path = os.path.join(output_dir, f"cloudy_example_{idx+1}_{int(cloud_percent)}.png")
